...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    try {
      const addresses = Taro.getStorageSync("historyAddresses");
      const mt = Taro.getStorageSync("myTrips");
      const mb = Taro.getStorageSync("myBookings");
      const role = Taro.getStorageSync("userRole");
      const routes = Taro.getStorageSync("savedRoutes");
      if (addresses) setHistoryAddresses(JSON.parse(addresses as string));
      if (mt) setMyTrips(JSON.parse(mt as string));
      if (mb) setMyBookings(JSON.parse(mb as string));
      if (role) setUserRoleState(role as UserRole);
      if (routes) setSavedRoutes(JSON.parse(routes as string));
      setRoleLoaded(true);
    } catch (e) {
      setRoleLoaded(true);
    }
  ...
  const publishTrip = useCallback(
    (tripData: Omit<Trip, "id" | "createdAt" | "isMyTrip" | "status">): Trip => {
      ...
      setMyTrips((prev) => {
        ...
        try {
          Taro.setStorageSync("myTrips", JSON.stringify(updated));
        } catch {}
        return updated;
      });
      return newTrip;
  ...
  const bookSeat = useCallback(
    (tripId: string, pickupPoint: string, dropoffPoint: string) => {
      ...
      setMyBookings((prev) => {
        ...
        try {
          Taro.setStorageSync("myBookings", JSON.stringify(updated));
        } catch {}
        return updated;
      });
  ...
  const approvePassenger = useCallback((requestId: string) => {
    ...
    setMyBookings((prev) =>
      prev.map((b) => (b.tripId === reviewRequest?.tripId ? { ...b, status: "confirmed" } : b))
    );
  }, [reviewRequest]);
  ...
  const rejectPassenger = useCallback((_requestId: string, _reason: string) => {
    ...
    setMyBookings((prev) =>
      prev.map((b) =>
        b.tripId === reviewRequest?.tripId && b.status === "waiting" ? { ...b, status: "rejected" } : b
      )
    );
  }, [reviewRequest]);
  ...
  const confirmBoarded = useCallback((passengerId: string) => {
    setDriverPassengers((prev) => prev.map((p) => (p.id === passengerId ? { ...p, status: "boarded" } : p)));
  }, []);
  ...
  const removePassenger = useCallback((passengerId: string) => {
    setDriverPassengers((prev) => {
      ...
      setTrips((trips) =>
        trips.map((t) =>
          myTrips.some((mt) => mt.id === t.id) ? { ...t, remainingSeats: t.remainingSeats + 1 } : t
        )
      );
  ...
  const addHistoryAddress = useCallback((addr: string) => {
    ...
    setHistoryAddresses((prev) => {
      ...
      try {
        Taro.setStorageSync("historyAddresses", JSON.stringify(updated));
      } catch {}
      return updated;
    });
  }, []);
  ...
  const addSavedRoute = useCallback((route: SavedRoute) => {
    setSavedRoutes((prev) => {
      ...
      try {
        Taro.setStorageSync("savedRoutes", JSON.stringify(updated));
      } catch {}
      return updated;
    });
  }, []);
  ...
  const publishPassengerRequest = useCallback(
    (reqData: Omit<PassengerRequest, "id" | "createdAt">): PassengerRequest => {
      ...
      setMyPassengerRequests((prev) => {
        ...
        try {
          Taro.setStorageSync("myPassengerRequests", JSON.stringify(updated));
        } catch {}
        return updated;
      });
  ...
  const acceptPassengerRequest = useCallback(
    (requestId: string, driverInfo: Omit<AcceptedByDriver, "acceptedAt">) => {
      ...
      setPassengerRequests((prev) => prev.map((r) => (r.id === requestId ? { ...r, acceptedBy: accepted } : r)));
      setMyPassengerRequests((prev) => {
        const updated = prev.map((r) => (r.id === requestId ? { ...r, acceptedBy: accepted } : r));
        try {
          Taro.setStorageSync("myPassengerRequests", JSON.stringify(updated));
        } catch {}
        return updated;
